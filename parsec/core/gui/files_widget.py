...
) -> tuple[FsPath, EntryID, dict[str, dict[str, object]], Optional[str]]:
    stats = {}
...
class FilesWidget(QWidget, Ui_FilesWidget):
    ...
    def reload(
        ...
        _ = self.jobs_ctx.submit_throttled_job(
            ...
            default_selection=default_selection.str if default_selection else None,
        )
    ...
    def _on_folder_stat_success(
        self, job: QtToTrioJob[tuple[FsPath, EntryID, dict[str, dict[str, object]], Optional[str]]]
    ) -> None:
        assert job.ret is not None
        ...
        if first_refresh:
            ...
            old_selection = set(x.name for x in self.table_files.selected_files())
