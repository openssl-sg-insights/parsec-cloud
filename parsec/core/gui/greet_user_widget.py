...
class Greeter:
    ...
    async def get_claimer_sas(self) -> Tuple[object, object]:
...
class GreetUserInstructionsWidget(QWidget, Ui_GreetUserInstructionsWidget):
    ...
    def __init__(self, jobs_ctx: Any, greeter: Any) -> None:
        super().__init__()
        self.setupUi(self)
    ...
    def _on_wait_peer_error(self, job: QtToTrioJob) -> None:
        ...
        if job.status != "cancelled":
            ...
            exc = None
            if job.exc:
                exc = job.exc.params.get("origin", None)  # type: ignore[attr-defined]
...
class GreetUserCheckInfoWidget(QWidget, Ui_GreetUserCheckInfoWidget):
    ...
    def __init__(
        self, jobs_ctx: Any, greeter: Any, user_profile_outsider_allowed: bool = False
    ) -> None:
        super().__init__()
        self.setupUi(self)
        self.jobs_ctx = jobs_ctx
...
class GreetUserWidget(QWidget, Ui_GreetUserWidget):
    ...
    def __init__(self, core: LoggedCore, jobs_ctx: Any, token: InvitationToken) -> None:
        super().__init__()
        self.setupUi(self)
        self.core = core
        self.jobs_ctx = jobs_ctx
    ...
    def _on_page_failed(self, job: QtToTrioJob) -> None:
        ...
        if job is not None and isinstance(job.exc.params.get("origin", None), BackendNotAvailable):  # type: ignore[union-attr]
            assert self.dialog is not None
            self.dialog.reject()
            return
        ...
        if job is not None and isinstance(
            job.exc.params.get("origin", None), InviteAlreadyUsedError  # type: ignore[union-attr]
        ):
            assert self.dialog is not None
            self.dialog.reject()
            return
        ...
        if job is not None and isinstance(
            job.exc.params.get("origin", None), InviteActiveUsersLimitReachedError  # type: ignore[union-attr]
        ):
            assert self.dialog is not None
            self.dialog.reject()
            return
    ...
    @classmethod
    def show_modal(cls, core: LoggedCore, jobs_ctx: Any, token: InvitationToken, parent: QWidget, on_finished: Callable[..., None]) -> GreetUserWidget:  # type: ignore[misc]
