...
class NavigationBarWidget(QWidget):
    ...
    def _on_label_clicked(self, idx: int) -> Callable[[str], None]:
        def _internal_label_clicked(_title: str) -> None:
    ...
    def from_path(self, workspace_name: EntryName, path: str) -> None:
        ...
        for idx, part in enumerate(parts):
            ...
            if idx == 0:
                ...
                self.layout().insertWidget(0, icon)  # type: ignore[attr-defined]
            else:
                icon.setFixedSize(15, 15)
                icon.setPixmap(QPixmap(":/icons/images/material/chevron_right.svg"))
                icon.apply_style()
                self.inner_widget.layout().insertWidget(  # type: ignore[attr-defined]
                    self.inner_widget.layout().count() - 1, icon
                )
            label: ClickableLabel | QLabel
            if idx != len(parts) - 1:
                obj_name = str(idx)
            ...
            label.setFont(font)
            self.inner_widget.layout().insertWidget(self.inner_widget.layout().count() - 1, label)  # type: ignore[attr-defined]
