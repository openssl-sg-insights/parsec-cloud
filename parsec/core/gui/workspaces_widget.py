...
from PyQt5.QtCore import QEvent, QObject, pyqtSignal, pyqtBoundSignal, Qt, QTimer
...
class WorkspacesWidget(QWidget, Ui_WorkspacesWidget):
    ...
    def __init__(self, core: LoggedCore, jobs_ctx: Any, event_bus: EventBus, **kwargs: Any):
        ...
        self.workspace_button_mapping: dict[Any, WorkspaceButton] = {}
    ...
    def goto_file_clicked(self) -> None:
        ...
        if button is not None:
            try:
                ...
                if not self.core.mountpoint_manager.is_workspace_mounted(button.workspace_fs, ts):  # type: ignore[arg-type]
    ...
    def on_list_success(self, job: QtToTrioJob) -> None:
        ...
        new_mapping: dict[Any, WorkspaceButton] = {}
    ...
    def remount_workspace_ts(self, workspace_fs: WorkspaceFS) -> None:
        def _on_finished(date: DateTime, time: DateTime) -> None:
            if not date or not time:
                return

            datetime = LocalDateTime(
                date.year(), date.month(), date.day(), time.hour(), time.minute(), time.second()  # type: ignore[operator]
            ).to_utc()
    ...
    def reencrypt_workspace(
        ...
        def _handle_fs_errors() -> Iterable[None]:
            try:
                yield
