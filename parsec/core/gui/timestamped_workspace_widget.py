...
class TimestampedWorkspaceWidget(QWidget, Ui_TimestampedWorkspaceWidget):
    ...
    def __init__(self, workspace_fs: WorkspaceFS, jobs_ctx: Any) -> None:
        super().__init__()
        self.setupUi(self)
        ...
        self.limits_job: Optional[Any] = self.jobs_ctx.submit_job(
    ...
    def _on_show_clicked(self) -> None:
        if self.dialog:
            ...
            cast(QDialog, QApplication.activeModalWidget()).accept()
        else:
    ...
    def on_error(self, job: QtToTrioJob) -> None:
    ...
    def on_success(self, job: QtToTrioJob) -> None:
        assert self.limits_job is job and self.limits_job is not None
        creation: LocalDateTime = self.limits_job.ret.to_local()
        self.limits_job = None
    ...
    @classmethod
    def show_modal(cls, workspace_fs: WorkspaceFS, jobs_ctx: Any, parent: QWidget, on_finished: Callable[..., Any]) -> TimestampedWorkspaceWidget:  # type: ignore[misc]
        ...
        def _on_finished(result: QDialog.DialogCode) -> Any:
